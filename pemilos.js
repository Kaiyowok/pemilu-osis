        ...
        // Countdown timer for voting period - PERBAIKAN: Mengubah tanggal menjadi di masa depan
        function startCountdown() {
            // Set the end date for voting (Mengubah ke tanggal di masa depan)
                    ...
                    candidates = [
                        {
                            ...
                            program: [
                                'Program mentoring akademik antar siswa',
                                'Festival seni dan budaya sekolah',
                                'Program OSIS peduli lingkungan'
                            ],
                        ...
                        },
                        {
                            ...
                            program: [
                                'Leadership training untuk pengurus OSIS',
        ...
        // Save vote to Firebase - PERBAIKAN: Menambahkan error handling yang lebih baik
        ...
        document.getElementById('login-form').addEventListener('submit', async function(e) {
            ...
            if (!nipd || nipd.length < 6 || !/^\d+$/.test(nipd)) {
                showError('NIS harus berupa minimal 6 digit angka');
                return;
            }
                ...
                candidateDiv.innerHTML = `
                    <div class="candidate-card card" data-candidate-id="${candidate.id}">
                        <div class="card-header text-center" style="position: relative; padding-bottom: 1.5rem;">
                            <div class="candidate-photo" style="background: linear-gradient(135deg, var(--primary-500), var(--blue-500)); display: flex; align-items: center; justify-content: center; color: white; font-size: 3rem;">
                        ...
                        <div class="card-content">
                            ...
                            <div class="tabs">
                                ...
                                <div id="program-${candidate.id}" class="tab-content">
                                    ...
                                    <ul style="list-style: none; padding: 0;">
                                        ${Array.isArray(candidate.program) ? candidate.program.map((program, index) => `
                                            <li style="font-size: 0.875rem; color: var(--gray-700); display: flex; align-items: flex-start; background: var(--success-50); padding: 0.75rem; border-radius: 0.5rem; border: 1px solid var(--primary-200); margin-bottom: 0.75rem;">
                                                <span style="width: 1.5rem; height: 1.5rem; background: linear-gradient(135deg, var(--success-600), var(--primary-600)); color: white; border-radius: 50%; display: flex; align-items: center; justify-content: center; font-size: 0.75rem; font-weight: bold; margin-right: 0.75rem; margin-top: 0.125rem; flex-shrink: 0;">
                                                    ${index + 1}
                                                </span>
                                                <span style="line-height: 1.6;">${program}</span>
                                            </li>
                                        `).join('') : '<p style="color: var(--gray-500); font-style: italic;">Program belum tersedia</p>'}
                                    </ul>
                                </div>
                            </div>
                        </div>
        ...
        // Submit vote - PERBAIKAN: Menambahkan event listener yang benar
        ...
        // Confirm vote - PERBAIKAN: Memperbaiki fungsi saveVote
        ...
        document.addEventListener('DOMContentLoaded', async function() {
            ...
            try {
                ...
                // Start countdown timer - PERBAIKAN: Mengubah tanggal menjadi di masa depan
                startCountdown();
